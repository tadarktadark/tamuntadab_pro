...
INSERT INTO TUPYO(TUPY_SEQ, TUPY_CLAS_ID, TUPY_TOTAL_USER, TUPY_STARTDATE, TUPY_ENDDATE, TUPY_STATUS)
	VALUES(TUPY_SEQ.NEXTVAL,1000000033, 6, CURRENT_DATE , TO_DATE('2023-09-07','YYYY-MM-DD'), 'P');
...
SELECT CLCH_ACCOUNT_ID ,CLCH_YEOKAL 
	...
	WHERE CLCH_CLAS_ID ='1000000033' AND CLCH_YEOKAL ='M' AND  CLCH_STATUS ='Y';

--클래스에 참여한 학생 조회
SELECT CLCH_ID,CLCH_ACCOUNT_ID ,CLCH_CLAS_ID ,CLCH_STATUS ,CLCH_GYEOLJE_STATUS ,CLCH_YEOKAL
	FROM CLASS_CHAMYEOJA cc
	WHERE CLCH_CLAS_ID ='1000000033';
...
FROM (
    ...
    LEFT JOIN (
        ...
        GROUP BY TUUS_OPTION_SEQ
    ) tu ON ot.TUOP_SEQ = tu.TUUS_OPTION_SEQ
    WHERE ot.TUOP_TUPY_SEQ = (
        SELECT TUOP_TUPY_SEQ
        FROM TUPYO_OPTION
        WHERE TUOP_SEQ = 1
    )
