...
SELECT COUNT(*)
	FROM (SELECT CORE_SEQ 
			...
			WHERE CORE_BOARD_ID = 'JI202309180056'
		UNION ALL
		SELECT SACR_SEQ 
			FROM SAKJE_COMMUNITY_REPLY
			WHERE SACR_BOARD_ID = 'JI202309180056');
...
SELECT SEQ, WRITER_ID, CONTENT , ROOT_SEQ, STEP, "DEPTH", CHAETAEK, REGDATE,"UPDATE", STATE
	FROM (SELECT SEQ, WRITER_ID, CONTENT , ROOT_SEQ, STEP, "DEPTH", CHAETAEK, REGDATE,"UPDATE",STATE,
				...
				FROM (SELECT CORE_SEQ AS SEQ, USER_NICKNAME AS WRITER_ID, 
							CASE WHEN CORE_STEP > 0 THEN LPAD(' ',1+(CORE_STEP*6)*3, '&nbsp;') || '<i class="ri-send-plane-2-fill"></i>' || CORE_CONTENT ELSE CORE_CONTENT END AS CONTENT , 
					...
					SELECT SACR_SEQ AS SEQ, 'DEL@' AS WRITER_ID , 
							CASE WHEN SACR_STEP > 0 THEN LPAD(' ',1+(SACR_STEP*6)*3, '&nbsp;') || '<i class="ri-send-plane-2-fill"></i>' || TO_CLOB('작성자에 의해 삭제된 글입니다.') ELSE TO_CLOB('작성자에 의해 삭제된 글입니다.') END AS CONTENT ,
							CASE (SELECT CORE_CHAETAEK 
								FROM COMMUNITY_REPLY
