SELECT SEQ, WRITER_ID, CONTENT , ROOT_SEQ, STEP, CHAETAEK, REGDATE,"UPDATE",STATE
			FROM (SELECT CORE_SEQ AS SEQ, USER_NICKNAME AS WRITER_ID, CORE_CONTENT AS CONTENT, 
					...
					WHERE CORE_BOARD_ID = 'JI202310030078'
						AND NOT CORE_STEP=0
				UNION ALL
				SELECT SACR_SEQ AS SEQ, 'DEL@' AS WRITER_ID , TO_CLOB('작성자에 의해 삭제되었습니다.') AS CONTENT,
					...
					WHERE SACR_BOARD_ID = 'JI202310030078'
						AND NOT SACR_STEP=0)
...
SELECT * FROM COMMUNITY_REPLY WHERE SUBSTR(CORE_BOARD_ID,1,2)='PI';

SELECT CORE_SEQ AS SEQ, CORE_BOARD_ID AS BOARD_ID, CLAS_TITLE AS WRITER_ID, CORE_CONTENT AS CONTENT, CORE_STATE AS STATE, CORE_WRITER_ID, 
						CASE WHEN PIBO_STATE = 'Y' THEN 'Y'
							WHEN PIBO_STATE = 'D' THEN 'D'
							WHEN CORE_BOARD_ID IS NULL THEN 'N' 
							WHEN PIBO_VIEW_GROUP = 'I' THEN 'I' 
							ELSE 'G' END AS CHAETAEK
						FROM COMMUNITY_REPLY LEFT JOIN PILGI_BOARD
						ON CORE_BOARD_ID = PIBO_ID
						JOIN CLASS ON PIBO_CLAS_ID = CLAS_ID
					UNION ALL
					SELECT CORE_SEQ AS SEQ, CORE_BOARD_ID AS BOARD_ID, JIBO_TITLE AS WRITER_ID, CORE_CONTENT AS CONTENT, CORE_STATE AS STATE, CORE_WRITER_ID, JIBO_STATE AS CHAETAEK
						FROM COMMUNITY_REPLY LEFT JOIN JILMUN_BOARD
						ON CORE_BOARD_ID = JIBO_ID
						WHERE CORE_WRITER_ID ='TMTD2';
			...
			FROM (SELECT ID, DAESANG_ID, DAESANG_CONTENT, ACCOUNT_ID, STATE, SAYU_ACCOUNT_ID, CATEGORY, CONTENT, REGDATE,
					...
					FROM (SELECT SIDA_ID AS ID, SIDA_DAESANG_ID AS DAESANG_ID, PIBO_CONTENT AS DAESANG_CONTENT, u1.USER_NICKNAME AS ACCOUNT_ID, SIDA_STATE AS STATE,
						...
						SELECT SIDA_ID AS ID, SIDA_DAESANG_ID AS DAESANG_ID, CORE_CONTENT AS DAESANG_CONTENT, u1.USER_NICKNAME AS ACCOUNT_ID, SIDA_STATE AS STATE,
							...
							ON SISA_ACCOUNT_ID = u2.USER_ACCOUNT_ID
							WHERE NOT SIDA_STATE='N')));
	WHERE rn BETWEEN (3-1)*5+1 AND 30*5
	ORDER BY rn;
...
SELECT CLAS_ID , CLAS_TITLE , CLAS_HYEONJAE_INWON, GAYE_ID, GAYE_STATE
...
(SELECT 'RE'||TO_CHAR(MAX(SUBSTR(GYEO_ID, 3))+1)
	FROM GYEOLJE);
SELECT 'RE'||TO_CHAR(NVL(MAX(SUBSTR(GYEO_ID, 3)),0)+1)
	FROM GYEOLJE
...
INSERT INTO GYEOLJE
	...
	VALUES((SELECT 'RE'||TO_CHAR(NVL(MAX(SUBSTR(GYEO_ID, 3)),0)+1)
			FROM GYEOLJE
			WHERE SUBSTR(GYEO_ID, 0, 2)='RE'), 60000, 'YY202309080006', 
...
UPDATE GANGEUISIL_YEYAK SET GAYE_GYEO_ID = ''
	WHERE GAYE_ID = '';
...
SELECT GYEO_GEUMAEK, GYEO_ACCOUNT_ID, USER_NICKNAME AS GYEO_ID, GYEO_STATUS, (SELECT GACO_NAME
	...
	WHERE GAYE_ID='YY202310010001';

SELECT GAYE_ID, GYEO_GEUMAEK
	FROM GANGEUISIL_YEYAK JOIN GYEOLJE
	ON GAYE_ID = GYEO_DAESANG_ID;


SELECT GAYE_GAGA_ID, (SELECT GACO_NAME
						FROM GANGEUISIL_GAEBYEOL JOIN GANGEUISIL_COMM
						ON GACO_ID = GAGA_GACO_ID
						WHERE GAYE_GAGA_ID = GAGA_ID) || ' '||
					(SELECT GAGA_NAME
...
SELECT CLAS_ID, CLAS_TITLE, CLAS_LOCATION, CLAS_HYEONJAE_INWON, CLAS_SUEOP_NALJJA, CLAS_SUBJECT_JEONGBO, CLAS_REGDATE, CLCH_ACCOUNT_ID, CLCH_STATUS, CLCH_YEOKAL
	FROM CLASS JOIN CLASS_CHAMYEOJA
	ON CLAs_ID = CLCH_CLAS_ID
	WHERE CLAS_STATUS IN ('매칭완료', '매칭')
		AND CLAS_ID IN ('1000000283', '1000000285')
	ORDER BY CLAS_ID;
...
SELECT ID, TITLE, ACCOUNT_ID
	FROM (SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE , PIBO_WRITER_ID AS ACCOUNT_ID
			...
			SELECT JABO_ID, JABO_TITLE , JABO_WRITER_ID AS ACCOUNT_ID
				FROM JAYU_BOARD)
	WHERE ID='PI202310020012';

SELECT CORE_BOARD_ID AS BOARD_ID, CORE_CONTENT AS CONTENT, CORE_WRITER_ID AS WRITER_ID
	FROM COMMUNITY_REPLY
	WHERE CORE_SEQ='17';
	
SELECT SIDA_DAESANG_ID 
	FROM SINGO_DAESANG
	WHERE SIDA_ID ='SI202310030028';
	

SELECT GYEO_STATUS, GACO_NAME, GAGA_NAME, GYEO_GEUMAEK, GAYE_YEYAK_DATE, GAYE_START_TIME, GAYE_HOURS
		FROM(SELECT GYEO_STATUS, GACO_NAME, GAGA_NAME, GYEO_GEUMAEK, GAYE_YEYAK_DATE, GAYE_START_TIME, GAYE_HOURS,
