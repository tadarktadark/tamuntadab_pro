...
SELECT PIBO_ID AS ID, PIBO_WRITER_ID AS WRITER_ID, CLAS_TITLE AS TITLE, PIBO_CONTENT AS CONTENT, c2.CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE
...
SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE ,PIBO_STATE AS STATE
	FROM (SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, PIBO_CLAS_ID, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE,PIBO_STATE,
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE || ' [' || PIBO_REPLY_COUNT || ']' AS TITLE, 
...
SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE || ' [' || PIBO_REPLY_COUNT || ']' AS TITLE, 
		...
		CASE WHEN PIBO_CLAS_ID IN (SELECT CLCH_CLAS_ID
								FROM CLASS_CHAMYEOJA
								WHERE CLCH_ACCOUNT_ID='TMTD2') THEN '1' ELSE '0' END AS DOWNLOAD_GROUP,
		CASE TO_CHAR(PIBO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_REGDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
...
-- SELECT UTL_MATCH.EDIT_DISTANCE_SIMILARITY(column1, column2) AS similarity
-- 	FROM your_table;
SELECT ID, TITLE
	FROM (SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE, 
			...
			SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE, 
				...
				WHERE PIBO_STATE = 'N'
					AND PIBO_VIEW_GROUP = 'A')
	WHERE ID IN (SELECT p2.PIBO_ID
						FROM (SELECT PIBO_ID, CLAS_SUBJECT_JEONGBO
								FROM PILGI_BOARD
								JOIN (SELECT CLAS_ID, REPLACE((SELECT JSON_ARRAYAGG(s.SUBJ_CODE) 
													     FROM JSON_TABLE(CLAS_SUBJECT_JEONGBO, '$[*]' COLUMNS (SUBJECT_CODE VARCHAR2(400) PATH '$')) jt 
													     JOIN (SELECT SUBJ_ID, SUBSTR(SUBJ_CODE, 3, LENGTH(SUBJ_CODE)-4) SUBJ_CODE
										...
										FROM CLASS)
								ON PIBO_CLAS_ID = CLAS_ID
								WHERE PIBO_ID ='PI202309130074') p1
						LEFT OUTER JOIN (SELECT 'PI202309130074' AS JOIN_ID, PIBO_ID, CLAS_SUBJECT_JEONGBO
											FROM PILGI_BOARD
											JOIN (SELECT CLAS_ID, REPLACE((SELECT JSON_ARRAYAGG(s.SUBJ_CODE) 
																     FROM JSON_TABLE(CLAS_SUBJECT_JEONGBO, '$[*]' COLUMNS (SUBJECT_CODE VARCHAR2(400) PATH '$')) jt 
																     JOIN (SELECT SUBJ_ID, SUBSTR(SUBJ_CODE, 3, LENGTH(SUBJ_CODE)-4) SUBJ_CODE
													...
													FROM CLASS)
											ON PIBO_CLAS_ID = CLAS_ID
											WHERE NOT PIBO_ID ='PI202309130074') p2
						ON p1.PIBO_ID = JOIN_ID
						WHERE UTL_MATCH.EDIT_DISTANCE_SIMILARITY(p2.CLAS_SUBJECT_JEONGBO, p1.CLAS_SUBJECT_JEONGBO) > 80
					UNION
					SELECT PIBO_ID
						FROM PILGI_BOARD
						WHERE PIBO_CLAS_ID IN (SELECT PIBO_CLAS_ID
													FROM PILGI_BOARD
													WHERE PIBO_ID='PI202309130074'))
		AND ROWNUM < 5;
--PI202309130003
...
UPDATE PILGI_BOARD SET PIBO_LIKE_USER = '{"TMTD2":"20230913","TMTD3":"20230913", "TMTD1":"20230913"}', PIBO_LIKE_COUNT = PIBO_LIKE_COUNT+1
	WHERE PIBO_ID='PI202309130012';
...
UPDATE PILGI_BOARD SET PIBO_VIEW_USER = '{"TMTD2":"20230913"}', PIBO_VIEW_COUNT = PIBO_VIEW_COUNT+1
	WHERE PIBO_ID='PI202309130012';
...
SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, DELFLAG, REGDATE, "UPDATE",PIBO_STATE AS STATE, SUBJECT_CODE
	FROM (SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, DELFLAG, REGDATE, "UPDATE",PIBO_STATE, SUBJECT_CODE ,
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE, PIBO_REPLY_COUNT AS REPLY_COUNT, PIBO_LIKE_COUNT AS LIKE_COUNT, 
						CASE NVL(JSON_VALUE(PILGI_BOARD.PIBO_LIKE_USER,'$.TMTD2'), '0') WHEN '0' THEN '0' ELSE '1' END AS LIKE_USER, PIBO_STATE AS DELFLAG,
						CASE TO_CHAR(PIBO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_REGDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
						CASE TO_CHAR(PIBO_UPDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_UPDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_UPDATE, 'yyyy. mm. dd') END AS "UPDATE",
...
SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, REGDATE, "UPDATE",PIBO_STATE AS STATE, CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE
	FROM (SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, REGDATE, "UPDATE", PIBO_STATE, PIBO_CLAS_ID,
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE, PIBO_REPLY_COUNT AS REPLY_COUNT, PIBO_LIKE_COUNT AS LIKE_COUNT, 
						CASE TO_CHAR(PIBO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_REGDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
						CASE TO_CHAR(PIBO_UPDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_UPDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_UPDATE, 'yyyy. mm. dd') END AS "UPDATE",
...
INSERT INTO PILGI_BOARD(PIBO_ID, PIBO_WRITER_ID, PIBO_CLAS_ID, 
			...
			VALUES((SELECT 'PI'||TO_CHAR(CURRENT_DATE,'yyyymmdd')||LPAD(MAX(TO_NUMBER(SUBSTR(PIBO_ID,11)))+1,4,'0')
						FROM PILGI_BOARD
						WHERE SUBSTR(PIBO_ID,1,10)='PI'||TO_CHAR(CURRENT_DATE,'yyyymmdd')), 'TMTD2', 1000000013, 
					'TEST 콘텐츠', 'A', 'A');
...
UPDATE COMMUNITY_REPLY SET CORE_CONTENT = '댓글 업데이트', CORE_UPDATE = SYSDATE
	WHERE CORE_SEQ='14';
...
SELECT JIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , JIBO_TITLE  AS TITLE,
		JIBO_CONTENT AS CONTENT, CLAS_TITLE  AS CLAS_ID,
...
SELECT ID, WRITER_ID , TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, CHAETAEK, REGDATE, STATE
	FROM (SELECT ID, WRITER_ID , TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, CHAETAEK, REGDATE, STATE,   
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , JIBO_TITLE || ' [' || JIBO_REPLY_COUNT || ']' AS TITLE,
...
SELECT JIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , JIBO_TITLE || ' [' || JIBO_REPLY_COUNT || ']' AS TITLE,
...
UPDATE JILMUN_BOARD SET JIBO_LIKE_USER = '{"TMTD1":"20230913"}', JIBO_LIKE_COUNT = JIBO_LIKE_COUNT+1
	WHERE JIBO_ID='JI202309130056';
...
UPDATE JILMUN_BOARD SET JIBO_VIEW_USER = '{"TMTD1":"20230913"}', JIBO_VIEW_COUNT = JIBO_VIEW_COUNT+1
	WHERE JIBO_ID='JI202309130056';
...
SELECT ID, TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, CHAETAEK, REGDATE, "UPDATE",STATE
	FROM (SELECT ID, TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, CHAETAEK, REGDATE, "UPDATE",STATE,
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JIBO_ID AS ID, JIBO_TITLE AS TITLE, 
						...
						(SELECT JSON_ARRAYAGG(s.SUTA_TITLE) 
						     FROM JSON_TABLE(JIBO_SUBJECT_CODE, '$[*]' COLUMNS (SUBJECT_CODE VARCHAR2(400) PATH '$')) jt
						     JOIN SUBJECT_TAG s ON jt.SUBJECT_CODE = s.SUTA_CODE) AS SUBJECT_CODE,
						JIBO_REPLY_COUNT AS REPLY_COUNT, JIBO_LIKE_COUNT AS LIKE_COUNT, 
						CASE NVL(JSON_VALUE(JILMUN_BOARD.JIBO_LIKE_USER,'$.TMTD1'), '0') WHEN '0' THEN '0' ELSE '1' END AS LIKE_USER,
...
SELECT ID, TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, CHAETAEK, REGDATE, "UPDATE"
	FROM (SELECT ID, TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, CHAETAEK, REGDATE, "UPDATE",
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JIBO_ID AS ID, JIBO_TITLE AS TITLE, 
						CLAS_TITLE  AS CLAS_ID,
						(SELECT JSON_ARRAYAGG(s.SUTA_TITLE) 
...
SELECT CLAS_ID, CLAS_TITLE, CLAS_SUBJECT_JEONGBO 
...
INSERT INTO JILMUN_BOARD(JIBO_ID, JIBO_WRITER_ID, JIBO_TITLE, 
			...
			VALUES((SELECT 'JI'||TO_CHAR(CURRENT_DATE,'yyyymmdd')||LPAD(MAX(TO_NUMBER(SUBSTR(JIBO_ID,11)))+1,4,'0')
						FROM JILMUN_BOARD
...
SELECT JABO_ID AS ID, USER_NICKNAME AS WRITER_ID , JABO_TITLE AS TITLE, JABO_CONTENT AS CONTENT
...
SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE, STATE
	FROM (SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE, STATE, 
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JABO_ID AS ID, USER_NICKNAME AS WRITER_ID , JABO_TITLE || ' [' || JABO_REPLY_COUNT || ']' AS TITLE,
...
SELECT JABO_ID AS ID, USER_NICKNAME AS WRITER_ID , JABO_TITLE || ' [' || JABO_REPLY_COUNT || ']' AS TITLE,
...
UPDATE JAYU_BOARD SET JABO_LIKE_USER = '{"TMTD1":"20230913"}', JABO_LIKE_COUNT = JABO_LIKE_COUNT+1
	WHERE JABO_ID='JA202313090072';
...
UPDATE JAYU_BOARD SET JABO_VIEW_USER = '{"TMTD1":"20230913"}', JABO_VIEW_COUNT = JABO_VIEW_COUNT+1
	WHERE JABO_ID='JA202313090072';
...
SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, REGDATE, "UPDATE", STATE
	FROM (SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, REGDATE, "UPDATE", STATE,
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JABO_ID AS ID, JABO_TITLE AS TITLE,
						JABO_REPLY_COUNT AS REPLY_COUNT, JABO_LIKE_COUNT AS LIKE_COUNT, JABO_STATE AS STATE,
						CASE NVL(JSON_VALUE(JAYU_BOARD.JABO_LIKE_USER,'$.TMTD1'), '0') WHEN '0' THEN '0' ELSE '1' END AS LIKE_USER,
						CASE TO_CHAR(JABO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(JABO_REGDATE, 'HH24:MI')ELSE TO_CHAR(JABO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
						CASE TO_CHAR(JABO_UPDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(JABO_UPDATE, 'HH24:MI')ELSE TO_CHAR(JABO_UPDATE, 'yyyy. mm. dd') END AS "UPDATE"
					FROM JAYU_BOARD
...
INSERT INTO JAYU_BOARD(JABO_ID, JABO_WRITER_ID, JABO_TITLE, JABO_CONTENT)
		VALUES((SELECT 'JA'||TO_CHAR(CURRENT_DATE,'yyyymmdd')||LPAD(MAX(TO_NUMBER(SUBSTR(JABO_ID,11)))+1,4,'0')
						FROM JAYU_BOARD
...
-- 신고 카테코리 조회
SELECT SICA_ID, SICA_EXPLANATION
	FROM SINGO_CATEGORY
	WHERE SICA_DELFLAG = 'N';

-- 신고 사유 입력
-- SIDA_ID 가져오기
SELECT SIDA_ID, SIDA_DAESANG_ID, SIDA_COUNT 
	FROM SINGO_DAESANG
	WHERE SIDA_STATE = 'N'
...
SELECT SIDA_ID, SIDA_DAESANG_ID, CORE_CONTENT, CORE_WRITER_ID, SISA_ACCOUNT_ID , SICA_EXPLANATION , SISA_CONTENT, SISA_REGDATE  
...
SELECT SIUS_SEQ, SIUS_ACCOUNT_ID , SIUS_COUNT , SIUS_STATE
	FROM SINGO_USER
	WHERE SIUS_ACCOUNT_ID = 'TMTD159';
-- 없으면
