...
SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE || ' [' || PIBO_REPLY_COUNT || ']' AS CLAS_ID, 
		PIBO_REPLY_COUNT AS REPLY_COUNT, CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE, 
		CASE NVL(JSON_VALUE(PILGI_BOARD.PIBO_LIKE_USER,'$.TMTD2'), '0') WHEN '0' THEN '0' ELSE '1' END AS LIKE_USER, PIBO_LIKE_COUNT AS LIKE_COUNT,
		PIBO_VIEW_COUNT AS VIEW_COUNT, 
...
SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, SUBJECT_CODE, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE
	FROM (SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, SUBJECT_CODE, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE,
...
SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE || ' [' || PIBO_REPLY_COUNT || ']' AS TITLE, 
		PIBO_CONTENT AS CONTENT, PIBO_REPLY_COUNT AS REPLY_COUNT, CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE, 
...
SELECT PIBO_ID, CLAS_TITLE || ' [' || PIBO_REPLY_COUNT || ']'  AS TITLE
	FROM PILGI_BOARD JOIN CLASS
	ON PIBO_CLAS_ID = CLAS_ID
	WHERE PIBO_CLAS_ID = 1000000082;
...
SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, DELFLAG, REGDATE, "UPDATE"
	FROM (SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, DELFLAG, REGDATE, "UPDATE",
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE, PIBO_REPLY_COUNT AS REPLY_COUNT, PIBO_LIKE_COUNT AS LIKE_COUNT, 
						CASE NVL(JSON_VALUE(PILGI_BOARD.PIBO_LIKE_USER,'$.TMTD2'), '0') WHEN '0' THEN '0' ELSE '1' END AS LIKE_USER, PIBO_DELFLAG AS DELFLAG,
						CASE TO_CHAR(PIBO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_REGDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
						CASE TO_CHAR(PIBO_UPDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_UPDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_UPDATE, 'yyyy. mm. dd') END AS "UPDATE"
					FROM PILGI_BOARD JOIN CLASS
					ON PIBO_CLAS_ID = CLAS_ID
...
SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, REGDATE, "UPDATE"
	FROM (SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, REGDATE, "UPDATE",
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT PIBO_ID AS ID, CLAS_TITLE AS TITLE, PIBO_REPLY_COUNT AS REPLY_COUNT, PIBO_LIKE_COUNT AS LIKE_COUNT, 
						CASE TO_CHAR(PIBO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_REGDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
						CASE TO_CHAR(PIBO_UPDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(PIBO_UPDATE, 'HH24:MI')ELSE TO_CHAR(PIBO_UPDATE, 'yyyy. mm. dd') END AS "UPDATE"
					FROM PILGI_BOARD JOIN CLASS
					ON PIBO_CLAS_ID = CLAS_ID
					WHERE NOT JSON_VALUE(PILGI_BOARD.PIBO_LIKE_USER,'$.TMTD2') IS NULL
						AND PIBO_DELFLAG='N'
						AND PIBO_VIEW_GROUP='C'
						AND PIBO_CLAS_ID IN (SELECT CLCH_CLAS_ID
												FROM CLASS_CHAMYEOJA
...
UPDATE PILGI_BOARD SET PIBO_DELFLAG = 'Y'
	WHERE PIBO_ID='PI202309130072';

--TDF019	TDW55431	필기 복원
UPDATE PILGI_BOARD SET PIBO_DELFLAG = 'N'
	WHERE PIBO_ID='PI202309130072';
...
SELECT COUNT(*)
	FROM (SELECT CORE_SEQ 
			...
			WHERE CORE_BOARD_ID = 'JI202309130077'
		UNION ALL
		SELECT SACR_SEQ 
			FROM SAKJE_COMMUNITY_REPLY
			WHERE SACR_BOARD_ID = 'JI202309130077');

SELECT SEQ, WRITER_ID, CONTENT , ROOT_SEQ, STEP, "DEPTH", CHAETAEK, REGDATE,"UPDATE"
	FROM (SELECT SEQ, WRITER_ID, CONTENT , ROOT_SEQ, STEP, "DEPTH", CHAETAEK, REGDATE,"UPDATE",
...
UPDATE COMMUNITY_REPLY SET CORE_STEP = CORE_STE+1
...
INSERT INTO COMMUNITY_REPLY(CORE_SEQ, CORE_BOARD_ID, CORE_WRITER_ID, CORE_CONTENT, 
						...
						CORE_STEP, 
						CORE_DEPTH)
			VALUES(COMMUNITY_REPLY_SEQ.NEXTVAL, 'JI202309130077', 'TMTD2', '댓글 입력 테스트', 
...
INSERT INTO SAKJE_COMMUNITY_REPLY(SACR_SEQ, SACR_BOARD_ID, SACR_ROOT_SEQ, SACR_STEP, SACR_DEPTH)
				VALUES(14, 'JI202309130077', 3, 7, 1);
...
SELECT ID, WRITER_ID , TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, CHAETAEK, REGDATE
	FROM (SELECT ID, WRITER_ID , TITLE, CLAS_ID, SUBJECT_CODE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, CHAETAEK, REGDATE,   
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
...
SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE
	FROM (SELECT ID, WRITER_ID , TITLE, REPLY_COUNT, LIKE_USER, LIKE_COUNT, VIEW_COUNT, REGDATE,   
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JABO_ID AS ID, USER_NICKNAME AS WRITER_ID , JABO_TITLE || ' [' || JABO_REPLY_COUNT || ']' AS TITLE,
						JABO_REPLY_COUNT AS REPLY_COUNT, 
...
SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, REGDATE, "UPDATE"
	FROM (SELECT ID, TITLE, REPLY_COUNT, LIKE_COUNT, LIKE_USER, REGDATE, "UPDATE",
			ROW_NUMBER() OVER(ORDER BY ID DESC) rn
			FROM (SELECT JABO_ID AS ID, JABO_TITLE AS TITLE,
						JABO_REPLY_COUNT AS REPLY_COUNT, JABO_LIKE_COUNT AS LIKE_COUNT, 
						CASE NVL(JSON_VALUE(JAYU_BOARD.JABO_LIKE_USER,'$.TMTD1'), '0') WHEN '0' THEN '0' ELSE '1' END AS LIKE_USER,
						CASE TO_CHAR(JABO_REGDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(JABO_REGDATE, 'HH24:MI')ELSE TO_CHAR(JABO_REGDATE, 'yyyy. mm. dd') END AS REGDATE, 
						CASE TO_CHAR(JABO_UPDATE, 'yyyymmdd') WHEN TO_CHAR(CURRENT_DATE, 'yyyymmdd') THEN TO_CHAR(JABO_UPDATE, 'HH24:MI')ELSE TO_CHAR(JABO_UPDATE, 'yyyy. mm. dd') END AS "UPDATE"
					FROM JAYU_BOARD
