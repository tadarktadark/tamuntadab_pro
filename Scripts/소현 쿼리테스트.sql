...
-- 총 게시물 개수
SELECT COUNT(*)
	FROM (SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE AS CLAS_ID , 
				CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE, PIBO_LIKE_USER AS LIKE_USER, PIBO_VIEW_USER AS VIEW_USER, 
				PIBO_VIEW_GROUP AS VIEW_GROUP, PIBO_DOWNLOAD_GROUP AS DOWNLOAD_GROUP, PIBO_REGDATE AS REGDATE
		...
		UNION ALL
		SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE AS CLAS_ID , 
				CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE, PIBO_LIKE_USER AS LIKE_USER, PIBO_VIEW_USER AS VIEW_USER, 
				PIBO_VIEW_GROUP AS VIEW_GROUP, PIBO_DOWNLOAD_GROUP AS DOWNLOAD_GROUP, PIBO_REGDATE AS REGDATE
			FROM PILGI_BOARD JOIN USER_PROFILE 
			ON PIBO_WRITER_ID = USER_ACCOUNT_ID 
...
-- 게시물 목록 조회
SELECT PIBO_ID AS ID, USER_NICKNAME AS WRITER_ID , CLAS_TITLE || ' [' ||(SELECT COUNT(*)
																			FROM COMMUNITY_REPLY
																			WHERE CORE_BOARD_ID = PIBO_ID) || ']' AS CLAS_ID, 
		CLAS_SUBJECT_JEONGBO AS SUBJECT_CODE, PIBO_LIKE_USER AS LIKE_USER, PIBO_VIEW_USER AS VIEW_USER, 
		PIBO_VIEW_GROUP AS VIEW_GROUP, PIBO_DOWNLOAD_GROUP AS DOWNLOAD_GROUP, PIBO_REGDATE AS REGDATE
	FROM PILGI_BOARD JOIN USER_PROFILE 
	ON PIBO_WRITER_ID = USER_ACCOUNT_ID 
