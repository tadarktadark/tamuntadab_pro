...
SELECT INPR_SEQ, INPR_ACCOUNT_ID,USER_PROFILE_FILE, USER_NICKNAME, INPR_CERT, USER_ACCOUNT_ID,
    SUBJECTS_TITLE, SUBJECTS_MAJOR_TITLE, REVIEW_COUNT,
    INPR_SUBJECTS, INPR_SUBJECTS_MAJOR, INPR_LIKE_COUNT, 
    INPR_VIEW_COUNT, INPR_REGDATE, USER_BIRTH,
	CASE WHEN NTILE(10) OVER (ORDER BY INPR_LIKE_COUNT DESC) = 1 THEN 'HOT' ELSE NULL END AS INGI
FROM (
SELECT INPR_ACCOUNT_ID, up.USER_PROFILE_FILE, INPR_CERT, INPR_SEQ, USER_ACCOUNT_ID,
	    up.USER_NICKNAME, USER_BIRTH,
	    (SELECT JSON_ARRAYAGG(s.SUTA_TITLE) 
...
GROUP BY up.USER_PROFILE_FILE, up.USER_NICKNAME, ip.INPR_SUBJECTS, ip.INPR_SUBJECTS_MAJOR, ip.INPR_LIKE_COUNT, INPR_ACCOUNT_ID, INPR_VIEW_COUNT, USER_ACCOUNT_ID, INPR_REGDATE, INPR_CERT, INPR_SEQ, USER_BIRTH
)
...
SELECT INPR_SEQ, INPR_ACCOUNT_ID, INPR_INTRO, USER_NICKNAME, USER_PROFILE_FILE, INPR_CERT, 
	CASE NVL(JSON_VALUE(INPR_LIKE, '$.TMTD1'), '0') WHEN '0' THEN '0' ELSE '1' END AS INPR_LIKE,
		(SELECT JSON_ARRAYAGG(s.SUTA_TITLE) 
	     FROM JSON_TABLE(INPR_SUBJECTS, '$[*]' COLUMNS (subject_code VARCHAR2(400) PATH '$')) jt
	     JOIN SUBJECT_TAG s ON jt.subject_code = s.SUTA_CODE) AS SUBJECTS_TITLE,
	     INPR_SITE_YOUTUBE, INPR_SITE_WEB, INPR_SITE_MOBILE
